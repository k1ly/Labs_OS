...
namespace HT
{
	...
	Element* NewElementGet(const void* key, int keylength)
	{
		return new Element(key, keylength);
	}

	Element* NewElementInsert(const void* key, int keylength, const void* payload, int  payloadlength)
	{
		return new Element(key, keylength, payload, payloadlength);
	}

	Element* NewElementUpdate(Element* oldelement, const void* newpayload, int  newpayloadlength)
	{
	...
	HTHANDLE* OpenExisting(const char FileName[512])
	{
		HTHANDLE* ht = nullptr;
		HANDLE fm = nullptr;
		LPVOID lp = nullptr;
