...
BOOL(*Update)(const HT::HTHANDLE*, const HT::Element*, const void*, int);
...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		OpenExisting = (HT::HTHANDLE * (*)(const char[])) GetProcAddress(htapi_dll, "Open");
		Update = (BOOL(*)(const HT::HTHANDLE*, const HT::Element*, const void*, int)) GetProcAddress(htapi_dll, "Update");
		...
		while (ht->Addr)
		{
			...
			int payload = 0;
			if (oldelement)
				payload = *(int*)(oldelement->payload) + 1;
			Print(oldelement ? oldelement : element);
			cout << (Update(ht, element, &payload, sizeof(payload)) ? "Updated!" : GetLastErrorMessage(ht)) << endl;
			delete element;
			Sleep(1000);
		}
